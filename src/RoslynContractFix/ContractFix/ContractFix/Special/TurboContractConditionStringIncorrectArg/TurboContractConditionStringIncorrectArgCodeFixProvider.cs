...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(TurboContractConditionStringIncorrectArgCodeFixProvider)), Shared]
    public class TurboContractConditionStringIncorrectArgCodeFixProvider : CodeFixProvider
    {
        ...
        public sealed override ImmutableArray<string> FixableDiagnosticIds
        {
            get
            {
                return ImmutableArray.Create(TurboContractConditionStringIncorrectArgAnalyzer.DiagnosticId);
            }
        }
        ...
        private static async Task<Document> ReplaceWithTurboContract(Document document, SyntaxNode nodeToReplace, string stringText, CancellationToken cancellationToken)
        {
            ...
            if (!ContractStatementAnalyzer.ParseInvocation(nodeToReplace.Parent as ExpressionStatementSyntax, out contractCallInfo) ||
                !contractCallInfo.IsSpecialContractType || !TurboContractConditionStringIncorrectArgAnalyzer.MethodNamesToFix.Contains(contractCallInfo.MethodNameAsString))
            {
                return document;
            }
            ...
            if (contractCallInfo.Message == null || contractCallInfo.IsConditionMatchConditionString)
            {
